...
The factor $S_t$ is needed in the Poisson distribution above to convert the units to the expected number of pickups on day $t$, which is the unit of interest. It can also be thought of as a normalizing factor that allows for day-to-day comparisons of daily pickup counts.
...
data <- data %>% 
  ...
  mutate(Pickups.per.hour = Pickups / (Total.ST.min / 60))

# Run log-linear model for pickups per hour comparing weekday/semester statuses
pickup.glm.2 <- glm(log(Pickups.per.hour) ~ Weekday + Semester,
                    data = data,
                    family = "poisson")
...
:   There is no evidence for significantly different behavior of daily pickups between weekdays and weekends, with the $p$-value for the corresponding coefficient being significantly greater than $\alpha = 0.05$, at $0.828$.

c.2

:   There is no evidence for significantly different behavior of daily pickups after the winter semester began, with the $p$-value for the corresponding coefficient being greater than $\alpha = 0.05$, at $0.128$.
